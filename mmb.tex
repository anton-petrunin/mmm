...
However, straightforward calculations show  $m(\alpha)=\frac \alpha {12}+ f(\alpha)$, where $f(\alpha)= O(\alpha^2)$ is a non-zero function.
% $f(\alpha)= O(\alpha ^3)$.
}
\end{ex}
...
\begin{quest}
Construct a {\color{green} closed}  manifold with a continuous Riemannian metric
