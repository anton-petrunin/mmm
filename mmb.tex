...
For $x\in X$ and $r>0$
denote by $B(x,r)$ the open metric ball of radius $r$ around $x$. Consider the Borel function $b_r\:X\to [0,\infty )$,
\begin{equation}
b_r(x):=\mu (B(x,r)).
\end{equation}
...
The map $r\mapsto v_r \in \mathrm M(X)$ measures the deviation of $X$ from $\R^n$.
...
the measures $v_r /r^2$ converge in $\mathrm M(X)$ to $\frac 1 {6(n+2)}\cdot scal \cdot \mu$, where
...
Then, for $r\to 0$, the measures $v_r/r$ converge in $\mathrm M(X)$ to
...
map $v_r\colon r\mapsto \mathrm M(X)$ describes the scalar curvature of the space.
...
v_r (K) = \frac 1 {\omega_n \cdot r^n}\cdot \Big(\mathcal M (T^r K)- \mathcal H^{2\cdot n} (U^r (K)) \Big).
\end{equation}
...
version of a theorem of M. Bonk and U. Lang, \cite{Bonk-Lang}, which relate the curvature measure to the volume of balls.
...
\begin{align*}
|v_r| (A\cap B (x_0,{2{\cdot}r}))
&\leq |v_r| (B (x_0,{2{\cdot}r})) \leq
\\
...
We deduce $|v_r| (A\setminus B (x_0,{2{\cdot}r})) \leq 6 {\cdot}\pi {\cdot}\delta _0 {\cdot} r^2$.
...
\tref{alexandrovthm}, it suffices to prove that any partial limit measure $\nu$ of a sequence $\frac 1 {r_j}\cdot v_{r_j}$ for $r_j\to 0$ must be the zero measure.
...
Then 
$$v_r (A) \leq C \cdot r \cdot \int _{B ( A,{2{\cdot}r})} |g' |.$$
\end{prop}
...
\[
v_r (A) \leq 2 \cdot C \cdot \mathcal N_0 (B (A,{3{\cdot}r}))\, .
\]
\end{cor}
...
$r<r_0$ we have 
$$v_r (A) \leq r\cdot 2\cdot  (n+2)\cdot C\cdot \mathcal N (A)\,. $$
\end{lem}
...
\item The natural coordinates $\phi$ can be smoothed out to satisfy the following property, \cite[Theorem B]{Otsu-Shioya}.
...
Borel subset $K\subset B (x,r)$ the deviation measure $v_s $ satisfies
...
Assume now that $\partial X\ne\emptyset$ and consider the doubling $Y=X\cup _{\partial X} X$ of $X$. Consider $X$ as a convex subset of $Y$ and let $K\subset X$ be compact. We find a constant $L>0$ such that for all sufficiently small $r>0$, we have 
$\mathcal H^n (K\cap B(r,{2{\cdot})} (\partial X)) \leq L \cdot r$. (This follows, for example, by the coarea formula using the Lipschitz properties of the gradient flow of the distance function $d(\cdot, \partial X)$ which is semiconcave). On the other hand, for $x\in K\setminus B (\partial X,r)$, the volumes of the $r$-ball in $X$ and in $Y$ coincide. Using that $Y$ has locally finite mm-boundary, we deduce that $v_r(K)$ (computed in the space $X$) is bounded from above $L\cdot r + \tilde v_r (K)$, where $\tilde v_r (K)$ is the deviation measure of $K$ considered as a subset of $Y$. This implies that $v_r /r$ is uniformly bounded for $r\to 0$. Thus $X$ has locally finite mm-boundary as well.
...
Let $K= \bar B (x,s) \cap \partial X$ be the closed ball of radius $s$ in $\partial X$ with respect to the ambient metric. Due to \cite[Section 1.6]{Evans},
...
any point $z\in B (K,{\frac 1 {10} r})$. Here $ k_1=k_1(n)>0 $ is a universal constant.
 Moreover, the set $B (K,{\frac 1 {10} r})$ has volume at least 
 $\frac 1 {20} \cdot r \cdot \omega _{n-1} \cdot s^{n-1}$.
Integrating over $V$ (and using the inequality of Bishop--Gromov on the complement of $B (K,{\frac 1 {10} r})$) we deduce:
