...
Let $U\subset X$ and $V\subset \R^n$ be open and assume that there is an $(1+\delta)$-bi-Lipschitz map $f\:U \to V$. %%%??? better e^{\pm\delta}-bi-Lipschitz
...
%the ball $B_{\frac r 4} (x)$ is $(1+C_n \delta )$-bi-Lipschitz to a Euclidean ball.
%\end{rem}
...
\cite{Bonk-Lang} provides us an $L$-bi-Lipschitz map $f\:\hat X\to \R^2$ with $L\leq 1+\frac {3\delta} {2\pi -3\delta}$.
...
once $\delta _0$ has been chosen small enough. Thus, we may assume
 that the tangent hyperplanes to points in $U$ are $\varepsilon_0$-close to the tangent
...
$$a :=\sup \{ |\nabla f (y)| \; ; y\in O \} \; .$$
If $\delta _0$ is small enough, then $a<\varepsilon_0$. The orthogonal projection $P\:U\to V$ is $1$-Lipschitz and the restriction of the inverse $P^{-1}$ to $O$ has Lipschitz constant $$\sqrt {1+ a^2} \leq 1+a^2 \leq 1+\varepsilon_0 ^2 \; .$$
...
Denote by $|D^2f|$ the largest eigenvalue of the Hessian $D^2f$. Since $f$ is convex and $\varepsilon _0$ small enough, the mean curvature $\mathcal \kappa (x)$ at the point $(x,f(x))$ of the graph $U$ of $f$ satisfies $\kappa (x) \geq \frac n 2 \cdot |D^2 f|$. Hence, the conclusion follows from the following statement.
...
We may assume that the bi-Lipschitz constant $1+\frac 1 C$ is close enough to 1, so that $\frac 1 2 < u <2$. Moreover, we may assume $B_x$ and $B^x$ are contained in the ball of radius $\frac 4 3 r$ around $x$ with respect to the background Euclidean metric.
...
at most $2\cdot (l_v^x -r)$, once the bi-Lipschitz constant $(1+\frac 1 C)$ is close enough to $1$.
...
We divide both sides by $\omega _n \cdot r^n$ and integrate over $A$. Using that the bi-Lipschitz constant is close enough to $1$, we see:
...
\item For any point $x\in X_{r,\delta}$ there are natural distance coordinates $\phi\: B_{3r} (x) \to \R^n$ which are $(1+\varepsilon)$-bi-Lipschitz onto an open subset $O \subset \R^n$. Here, $\varepsilon\to 0$ as $A\to\infty$, \cite[9.4]{BGP}.
...
Any limit of a sequence $v_{r_j} /r_j$ for some $r_j\to 0$ must be again non-negative, hence a Radon measure. Outside of $\partial X$
 ...
 $(1+\varepsilon)$-bi-Lipschitz to a half-ball in the Euclidean space. Choose an arbitrary $s>0$ such that $B_{2s} (x)\subset U$.
	...
	In order to prove this inequality, we consider any open neighborhood $V$ of $K$ in $X$. For all small $r>0$, the neighborhood $V$ contains $B_{2r} (K)$. Once $\varepsilon$ has been chosen small enough, the ball $B_r (z)$ in $X$ has volume at most
...
%the ball $B_{\frac r 4} (x)$ is $(1+C \cdot \delta )$-bi-Lipschitz to a Euclidean ball.
\end{quest}
