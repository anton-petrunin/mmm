...
Let $X$ be  a countably $n$-rectifiable {\color{red} metric space},
 and assume that {\color{red}$\mu$ is a Radon measure which is } non-zero on open subsets of $X$.
 Then for $\mu =\mathcal H^n$ the functions $v_r$ converge $\mathcal H^n$-almost everywhere to $0$. Moreover, $\mu=\mathcal H^n$  is the only measure  with this property~\cite{AmbKirk}[Theorem 5.4].
  \footnote{That's the correct reference, right? Vitya}
 \end{ex}
...
As seen in Example \ref{ex:rect} and is well-known from many important classes of spaces, most points in {\color{red} reasonably nice} spaces are rather regular.
...
the details, however {\color{red} a rigorous  proof can be extracted from the proof }of \tref{alexandrovthm} in Section \ref{sec:Alex} below.
