...
The curvature at the tip of the cone is $\alpha=2{\cdot}\pi-\rho$.
The mm-curvature is a Dirac measure of mass $m(\alpha)$ concentrated in the tip.
Straightforward calculations show that $m(\alpha)=\frac \alpha 6+ f(\alpha)$, where $f(\alpha)= O(\alpha^2)$ is a non-zero function.
% $f(\alpha)= O(\alpha ^3)$.
}
\end{ex}
...
{\color{red} Any  $\mu \in M(X)$ can be uniquely written as $\mu_+-\mu_-$, where $\mu_\pm$ are a Radon measures. The  measure $|\mu|=\mu_+-\mu_-$ is called \emph{the total variation} of $\mu$.}
...
{\color{red} The limit of }any converging sequence of $k$-geodesics is a $k$-geodesic.
...
{\color{red} It remains to prove the Claim.}
