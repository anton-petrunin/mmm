...
\[\mathcal K(B_r(y)) \leq \delta \cdot r^{n-1}\]
for any $y\in B_{t} (A)$ and any $0<r<t$.
\end{lem}
...
<
C\cdot \delta \cdot \mathcal K( B_{6{\cdot}r} (x)) \cdot r^{1-n}.
\end{equation}
\end{lem}
...
The mean curvature vanishes on $B_6 (x)$ if an only if $B_6(x)$ is contained in a flat hyperplane.
...
Denote by $B_2(x)_W$ the ball of radius $2$ in $W$ around $x$. 
Set
$$a :=\sup \set{|\nabla f (y)|}{y\in B_2(x)_W}.$$
If $\delta_0$ is small, then $a<\varepsilon_0$.
The orthogonal projection $P\:U\to V$ is $1$-Lipschitz and the restriction of the inverse $P^{-1}$ to $B_2(x)_W$ has Lipschitz constant $$\sqrt {1+ a^2} \leq 1+a^2 \leq 1+\varepsilon_0 ^2.$$
...
Fix a compact subset $A\subset Y$ and let $\delta>0$ be an arbitrary sufficiently small number. Consider a positive $1>t>0$ provided by \lref{lem:compsm}. Let $U$ be the open set $B_t (A)$. Making $t$ smaller, if needed, we may assume that $\mathcal K(B_{7{\cdot}t}(A))$ is bounded by a
...
%If $\omega _n{\cdot}r^n -\mathcal H^n (B_r (x)) \leq \delta \cdot r ^n$, then
