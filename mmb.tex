...
In both cases we can find a compact subset $K_1\subset X_{reg}$ and (by homogeneity) a Borel subset $A\subset T^1 K_1$ such that either $\phi_1$ is not defined on almost all of $A$ or $\mathcal M (A) > \mathcal M(\phi (A))$.
Replacing $A$ by a smaller subset, if necessary, we find in both cases some $\varepsilon >0$, such that $$\varepsilon < \mathcal M (A) - \mathcal M(\phi_1 (A)).$$
...
\\
&\leq\mathcal M ( A_m) - \mathcal M( \phi _1 (A_m\cap N^r)).
\end{align*}
Setting $S_m=A_m\cap N^{r_m}$ we estimate the right hand side as the sum of the following three terms:
\begin{align*}
...
%$4r\leq r_0$. Let $K$ be a positive measure on $X$. Let $A\subset X$ be given, with finite $K (B_{2{\cdot}r} (A))$.
%Then $\int _A K(B_r (x)) d\vol (x) \leq \bar L r^n \cdot K (B_{2{\cdot}r}(A))$.
%\end{lem}

%\begin{proof}
%Cover $A$ by balls $B_i$ of radius $2{\cdot}r$, such that the balls with the same centers and radii $r$ still cover $A$ and such that
...
%$$\leq \Sigma K(B_i) L (2{\cdot}r)^n \leq 2^n L L' K(B_{2{\cdot}r} (A))$$
%\end{proof}
...
Then $$|1- \frac {b_r(x)} {\pi{\cdot}r^2} | \leq 3 \cdot |\Omega |( B_{r} (x)).$$
\end{lem}
...
Let $A\subset X$ be a compact subset. Choose some $\varepsilon >0$ such that the closure of $B_{2\varepsilon} (A)$ in $X$ is compact and such that,
for any $0<2{\cdot}r<\varepsilon$ the inequality $\mathcal H^2 (B_{2{\cdot}r}(x_0)) < \frac 1 {\varepsilon} r^2$ holds true; see also \cite[Lemma 8.1.1]{Reshetnyak-GeomIV}.

Let $r<\varepsilon$ be arbitrary.
For any $x\in B_{2{\cdot}r} (x_0)$ we have $$b_r (x) =\mathcal H^2 (B_r (x)) \leq \mathcal H^2 (B_{3r} (x_0)) \leq \frac 1 {\varepsilon} r^2.$$
For any $x\notin B_{r} (x_0)$ we have $| \Omega |(B_r (x)) < \delta _0$. Thus, by Lemma~\ref{lem:bl}, $$|1-\frac {b_r(x)} {\pi{\cdot}r^2} | \leq 3 |\Omega | ( B_{r} (x)).$$
For the deviation measures $v_r$ from \eqref{eq:first} we estimate:
$$|v_r| (A\cap B_{2{\cdot}r} (x_0))) \leq |v_r| (B_{2{\cdot}r} (x_0)) \leq (1+ \frac 1 {\varepsilon}) \cdot \mathcal H^2 (B_{2{\cdot}r} (x_0)) \leq
(1+ \frac 1 {\varepsilon}) \cdot \frac 1 {\varepsilon} r^2.$$
On the other hand,
%for any $x\in A\setminus B_{r} (x_0)$ we have $\bar \Omega (B_r (x)) < \delta _0$, hence $|\frac {b_r(x)} {\pi{\cdot}r^2} -1| \leq 5 |\Omega | (\bar B_{r} (x))$. Therefore,
$$|v_r| (A\setminus B_{2{\cdot}r} (x_0)) \leq \int _{A \setminus B_{2{\cdot}r} (x_0) } 3 \cdot |\Omega| (B_r (x)) \cdot d\mathcal H^2 (x) \leq $$
$$\leq 3 \int _{B_r (A\setminus B_{2{\cdot}r} (x_0))} \mathcal H^2 (B_r(x)) \cdot d|\Omega | (x),$$
%where $Q= B_r (A\setminus B_{2{\cdot}r} (x_0)$, and
...
$$f_z (t)= f(z- \frac t 3 y_0 ) \;, t\in [0,6]$$
...
$$\leq C_4 \int _{B_{2{\cdot}r} (A)} \Big ( \int _{|x-y|< \frac 4 3 r} |g'(y)| \cdot |y-x|^{1-n} \cdot d\mathcal L^n (x) \Big ) \cdot d\mathcal L^n (y) = $$
$$= \frac 4 3 C_4 \int _{B_{2{\cdot}r} (A)} |g'(y)| \cdot r \cdot d\mathcal L^n (y),$$
